...
public class Kinematics : MonoBehaviour {
	...
	//Called when the go button is pressed  按下go按钮的调用
	public void StartKinematics()
	{
		if(time.text != null && inputX.text != null && inputY.text != null && inputZ.text != null)
		{
			target = new Vector3(float.Parse(inputX.text), float.Parse(inputY.text), float.Parse(inputZ.text));  //float.Parse将输入值转为float型
